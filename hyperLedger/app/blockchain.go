...
func useWalletGateway() {
	...
	// invoke
	fmt.Println("\n\n --- invoke ---")
	resultInvoke, err := contract.SubmitTransaction("invoke", "65792454091", "1", "1", "0", "1", "1", "1", "1", "1", "1", "0")
	if err != nil {
		fmt.Printf("Failed to commit transaction: %v", err)
	} else {
	...
	// Get 0
	fmt.Println("\n --- Get0 ---")
	result, err := contract.SubmitTransaction("query", "65792454094")
	if err != nil {
		fmt.Printf("Failed to commit transaction: %v", err)
	} else {
		fmt.Println("Commit is successful - GET0")
		var smart DeviceChaincode
